...
class MDN_Copula(nn.Module):
    ...
    # torch.backends.cuda.matmul.allow_tf32 = True   # 矩阵乘
    ...
    def sample(self, x, max_attempts=50):
        ...
        #     testout=torch._dynamo.explain(self)(x)
        #     print(testout)
        #     self.printed=True
        pi_logits, t_rates, q_means, q_stds, rho = self.forward(x)
        ...
        # 获取选中分量的参数
        rate = t_rates[idx, comp]          # (B,)
        mu = q_means[idx, comp]             # (B,)
        sig = q_stds[idx, comp]             # (B,)
        r = rho[idx, comp]                  # (B,)
        ...
        # 数值稳定性：裁剪相关系数，防止 sqrt(1-r^2) 出现负值
        r = torch.clamp(r, -1 + 1e-7, 1 - 1e-7)

        for _ in range(max_attempts):
            # 提前退出检查（一次同步，但通常能减少无用迭代）
            if not need.any().item():
                break

            # 生成全批量独立标准正态样本（避免动态形状）
            z1 = torch.randn(B, device=device)
            z2 = torch.randn(B, device=device)

            # 解析法生成相关标准正态样本（等效于 N(0, Σ)）
            # z1 不变，z2 变换为 r*z1 + sqrt(1-r^2)*z2
            z2_corr = r * z1 + torch.sqrt(1 - r**2) * z2

            # 计算 t 候选值（逆变换采样，使用 ndtr 加速 CDF）
            u = torch.special.ndtr(z1)                     # 标准正态 CDF
            u = torch.clamp(u, 0.0, 1.0 - 1e-7)            # 避免 log(0)
            t_cand = -torch.log(1.0 - u) / rate

            # 计算 q 候选值
            q_cand = mu + sig * z2_corr

            # 仅对 need=True 的位置进行更新
            t = torch.where(need, t_cand, t)
            q = torch.where(need, q_cand, q)

            # 更新需要重采样的掩码
            need = (q < 0)

        # 对最终仍为负的 q 设置小正值
        q = torch.where(need, torch.full_like(q, 0.01), q)
